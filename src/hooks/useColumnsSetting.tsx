...
export const useColumnsSetting = <T,>(columns: ComponentsType.ColumnType<T>[], uniqueKey: string) => {
  ...
  const SettingBtn = useMemo(() => <ColumnsSetting source={source} select={select as string[]} methods={methods} />, [source, select, methods]);

  const columnsFilter = useMemo(() => columns.filter(item => select?.includes(item.key as string)) as AntdType.ColumnsType<T>, [columns, select]);
  return {
