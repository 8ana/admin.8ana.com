...
const Header = (props: HeaderProps) => {
  ...
  return (
    <div className={styles.header}>
      <Checkbox indeterminate={!!select.length && select.length < length} checked={select.length === length} onChange={methods.allToggle}>
...
export const columnsSorter = (
  ...
  key: string,
  preset = false
): Pick<ComponentsType.ColumnType<unknown>, 'dataIndex' | 'key' | 'preset' | 'align' | 'width' | 'sorter' | 'sortOrder'> => {
  return {
    dataIndex: key,
    key,
    preset,
    ...
    width: 120,
    sorter: true,
    sortOrder: state.sortBy === key ? (state.sortOrder === 1 ? 'ascend' : state.sortOrder === -1 ? 'descend' : null) : null,
  };
};
...
export const columnsFilter = (
  ...
  key: string,
  preset = false
): Pick<ComponentsType.ColumnType<unknown>, 'dataIndex' | 'key' | 'preset' | 'align' | 'width' | 'filters' | 'filtered' | 'filteredValue' | 'filterMultiple' | 'render'> => {
...
export const columnsFilterRequest = (
  ...
  'dataIndex' | 'key' | 'preset' | 'align' | 'width' | 'filters' | 'filtered' | 'filteredValue' | 'filterMultiple' | 'filterDropdown' | 'render'
> => {
  const stateFilterValue = state[key as keyof CommonType.ListQuery];
  return {
    dataIndex: key,
