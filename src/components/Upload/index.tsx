...
export const UploadImg: React.FC<ImgPropsType> = props => {
  const { accept = 'image/jpeg,image/jpg,image/png,image/gif,image/x-icon,image/vnd.microsoft.icon', type = 'others', value, onChange, width = 128, height = 128 } = props;
  ...
  return (
    <Upload name="file" showUploadList={false} accept={accept} data={{ type }} beforeUpload={beforeUpoad} customRequest={customRequest}>
      {value && (
        <div className={styles.imgWrapper} style={{ width, height }} onClick={e => e.stopPropagation()}>
