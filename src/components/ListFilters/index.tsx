...
interface PropsType {
  ...
  confirm: (params: AntdType.FilterConfirmProps) => void;
  clearFilters?: () => void;
}
export const ListFilters: React.FC<PropsType> = props => {
  ...
  const onChangeCall = useCallback((e: AntdType.RadioChangeEvent) => setValue(e.target.value), [setValue]);
  ...
  const data = useMemo(() => source.filter(item => new RegExp(query).test(item.label)), [source, query]);
