...
export const useMethods = (select: string[], setSelect: (values: string[]) => void) => {
  ...
  const queryCompare = useCallback(
    (value: Partial<CommonType.ListQuery>) => {
      const init = Object.entries(value).some(item => state[item[0] as keyof CommonType.ListQuery] !== item[1]);
      init && setState(value);
    },
    [setState, state]
  );
  ...
  const remove = useCallback(async (id: string) => await actions.deleteEposideItem({ id }), [actions]);
  ...
  const update = useCallback(async (values: EposideType.UpdateItemReq) => await actions.updateEposideItem({ ...values }), [actions]);
  ...
  const methods = useMemo(() => ({ init, set: queryCompare, update, updateMany, remove, removeMany, reset }), [init, queryCompare, update, updateMany, remove, removeMany, reset]);
