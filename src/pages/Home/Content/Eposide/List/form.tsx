...
export const EditForm = forwardRef<FormInstance<unknown>, PropsType>((props, ref) => {
  ...
  return (
    <Form ref={ref} {...restProps}>
      {data?.id && (
        <Form.Item label={getLang('animate.title')} name="title" initialValue={data?.title} rules={[{ required: true }]}>
          <Input placeholder={getLang('animate.title.tips')} />
        </Form.Item>
      )}
      {data?.id && (
        <Form.Item label={getLang('animate.slug')} name="slug" initialValue={data?.slug} rules={[{ required: true }]}>
      ...
      <Form.Item label={getLang('animate.update')} name="isUpdate" initialValue={data?.isUpdate}>
        <Select placeholder={getLang('animate.status.tips')}>
          <Select.Option value={0}>{getLang('common.select.updating')}</Select.Option>
          <Select.Option value={1}>{getLang('common.select.updated')}</Select.Option>
        </Select>
      </Form.Item>
