...
export const useColumns = (state: CommonType.ListQuery, methods: CommonType.ListMethods<AnimateType.UpdateItemReq>) => {
  const columnsOrigin: ComponentsType.ColumnsType<AnimateType.List> = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (text, record) => (
          <Space>
            <QuickEdit<AnimateType.List, AnimateType.UpdateItemReq> data={record} submit={methods.update} init={methods.init}>
              <EditForm />
            </QuickEdit>
  ...
  const { columns, SettingBtn } = useColumnsSetting<AnimateType.List>(columnsOrigin, 'animateColumns');
