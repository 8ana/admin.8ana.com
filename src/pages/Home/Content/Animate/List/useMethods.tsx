...
export const useMethods = (initialState: CommonType.ListQuery) => {
  ...
  const remove = useCallback(async (id: string) => await actions.deleteAnimateItem({ id }), [actions]);
  ...
  const update = useCallback(async (values: AnimateType.UpdateItemReq) => await actions.updateAnimateItem({ ...values }), [actions]);
